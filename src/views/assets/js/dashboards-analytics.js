...
fetch("/src/controllers/api.php?info=nutrients").then(response => response.json()).then(data => {
  ...
  const chartOrderStatistics = document.querySelector('#orderStatisticsChart'),
    orderChartConfig = {
      ...
      labels: ['Calorías', 'Carbohidratos', 'Grasa', 'Proteínas'],
      series: [data.calorias, data.carbohidratos, data.grasa, data.proteinas],
    ...
    const resultData = Object.values(data).every(e => e == 0);
    if(!resultData){
      document.getElementById("alert-consume").style.display = "none";
      ...
      document.getElementById("alert-consume").style.display = "block";
    }
...
fetch("/src/controllers/api.php?info=weight").then(response => response.json()).then(data => {
  ...
  const incomeChartEl = document.querySelector('#incomeChart'),
    ...
    if (seriesData.length <= 1) {
      incomeChartConfig.markers.discrete[0].dataPointIndex = 0;
    }
